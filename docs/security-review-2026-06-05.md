...
현재 MVP는 커플 멤버 권한, 관리자 권한, 리드 마스킹, 주민등록번호 입력 차단, 분석 payload 개인정보 차단, 기본 rate limit, CORS allowlist, 보안 헤더의 기본 방어선을 갖췄다. 다만 운영 출시 전에는 토큰 회전/폐기, 분산 rate limit 저장소, 파일 저장소 접근 제어, 결제 idempotency 설계가 반드시 보강되어야 한다.

## 리스크 목록

| 심각도 | 영역          | 리스크                                                                                                                                         | 재현 방법                                                                                                    | 수정 제안                                                                                                                 |
| ------ | ------------- | ---------------------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------ | ------------------------------------------------------------------------------------------------------------------------- |
| High   | 인증 토큰     | accessToken 만료는 있으나 refresh token, 서버 측 폐기 목록, 키 회전 절차가 없다. 유출 시 만료 전까지 재사용될 수 있다.                         | 발급된 토큰을 localStorage에서 복사해 다른 브라우저에서 `Authorization` 헤더로 사용한다.                     | refresh token, 서버 측 세션 폐기 목록, 비밀키 회전 절차를 추가한다.                                                       |
| High   | Rate limit    | 기본 rate limit은 있으나 서버리스 인스턴스별 메모리 저장소에 의존한다. 분산 환경에서 제한이 느슨해질 수 있다.                                  | 여러 인스턴스/Origin에서 `/auth/login`, `/analytics/events`, `/compare-rooms/{id}/lead-requests`를 호출한다. | Redis/KV 기반 IP+계정+토큰 rate limit, 초대 토큰 실패 횟수 제한, 리드 신청 중복/쿨다운을 추가한다.                        |
...
| Medium | CSRF          | API가 Bearer 토큰 기반이라 기본 CSRF 위험은 낮고 운영 CORS allowlist가 있다. 다만 관리자 origin 분리와 credentials 정책은 더 명확히 해야 한다. | 임의 Origin에서 API 요청을 만들고 preflight 허용 여부를 확인한다.                                            | 관리자 앱 별도 origin 제한, credentials 정책 명시, CORS 테스트를 배포 검증에 포함한다.                                    |
...
- 운영 `AUTH_TOKEN_SECRET` 비밀 관리와 토큰 회전/폐기 정책 확정.
- CORS origin allowlist와 보안 헤더 배포 검증.
- 인증/초대/리드/분석 rate limit을 분산 저장소 기반으로 강화.
