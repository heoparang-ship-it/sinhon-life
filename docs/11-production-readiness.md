...
- production Vercel build는 `pnpm db:generate` 후 `VERCEL_ENV=production`에서만 `DIRECT_URL`로 호환 스키마 기준점 처리, 필요 시 체크인된 migration SQL bootstrap, DB migration, seed를 실행한다. 기존 public schema의 다른 테이블은 삭제하거나 강제 동기화하지 않는다.
