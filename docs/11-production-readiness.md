...
- production Vercel build는 `pnpm db:generate` 후 `VERCEL_ENV=production`에서만 `DIRECT_URL`로 호환 스키마 기준점 처리, 필요 시 Supabase `auth` schema를 인식한 비파괴 `db push` bootstrap, DB migration, seed를 실행한다.
