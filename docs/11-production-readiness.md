...
- production Vercel은 기존 public schema와 충돌하지 않도록 DB schema를 `sinhon_os`로 강제한다.
- production Vercel build는 `pnpm db:generate` 후 `VERCEL_ENV=production`에서만 `DIRECT_URL`로 schema 생성, 호환 스키마 기준점 처리, 필요 시 비파괴 `db push` bootstrap, DB migration, seed를 실행한다.
