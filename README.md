...
Vercel production build는 `pnpm db:generate` 후 `VERCEL_ENV=production`일 때만 `scripts/vercel-production-db-prepare.mjs`로 `DIRECT_URL`을 사용해 호환 스키마 기준점 처리, 필요 시 비파괴 `db push` bootstrap, `migrate deploy`, seed를 실행합니다.
