...
type QuickItem = {
  ...
  href: string;
  badge?: string;
  category?: DecisionCategory | "ai" | "support" | "cheongmo";
        ...
        <div className="mx-5 mt-[22px] grid grid-cols-4 rounded-[18px] border border-[#EEF2F6] bg-white py-5 shadow-[0_1px_2px_rgba(26,36,51,0.04),0_10px_24px_rgba(26,36,51,0.045)]">
          {QUICK_ITEMS.map((q, i) => (
            <Link
              ...
              href={q.href}
              onClick={() => {
                if (q.category === "support") track("support_entry_click", { from: "home_quick" });
                else if (q.category === "cheongmo") track("cheongmo_entry_click", { from: "home_quick" });
                else track("decision_card_click", { category: q.category ?? null, from: "home_quick" });
              }}
